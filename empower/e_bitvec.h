 ...
 * is not really necessary in most cases. So technically, these should be called "bit arrays"
...
/**
 * A bit vector, consisting of the \data pointer and \cap, the number of available bits.
 */
typedef struct {
    unsigned char *data;
    size_t cap;
...
/**
 * Initialise a bit vector with a pointer `data` that allows storing `cap` BITS (not bytes!) of
 ...
 *
 * The range is given by `start` (inclusive) and `end` (exclusive). If all bits within that range
 * are 1, 1 is returned, 0 otherwise.
 *
 ...
 *
 * The range is given by `start` (inclusive) and `end` (exclusive). If at least one bit within that
 * range is 1, 1 is returned, 0 otherwise.
 *
 * If `start` is out of range, 0 is returned. If `end` is out of range, it is trimmed to the
