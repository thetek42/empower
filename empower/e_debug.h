...
/**************************************************************************************************
 *
 * Empower / e_debug.h - Public Domain - https://git.tjdev.de/thetek/empower
 *
...
/**
 * Debug-print the generic argument `value`. For a call like `e_debug (x + y)`, it will print `x + y
 * = 42` with the regular debug format from e_log, where 42 is the result of the addition. Works
 * with every type that is accepted by `E_DEBUG_AUTO_FMT`.
 */
...
#  define REALLOC_ERROR_MSG(oldptr, size, location)                                                \
      "[e_debug] failed to reallocate %s to %zu bytes at %s\n", oldptr, size, location
#  define SUCCESS_MSG(ptr, size, location)                                                         \
      "[e_debug] allocated %zu bytes at %s => %p\n", size, location, ptr
#  define REALLOC_SUCCESS_MSG(oldptr, newptr, size, location)                                      \
      "[e_debug] reallocated %s to %zu bytes at %s => %p\n", oldptr, size, location, newptr
# else
#  define ERROR_MSG(size, location) "[e_debug] failed to allocate at %s\n", location
#  define REALLOC_ERROR_MSG(ptr, size, location)                                                   \
      "[e_debug] failed to reallocate %p at %s\n", ptr, location
#  define SUCCESS_MSG(ptr, size, location) "[e_debug] allocated at %s => %p\n", location, ptr
#  define REALLOC_SUCCESS_MSG(ptr, newptr, size, location)                                         \
      "[e_debug] reallocated %p at %s => %p\n", ptr, location, newptr
# endif
