 ...
 *
 * The decoded string of length `encoded_len` has to be given `encoded_input`. The output will be
 * placed in `output`, and the length of the decoded output will be written to `output_len`. The
 * user must ensure that `output` is big enough. For finding out how much space is required for the
 * output, the function `e_cobsr_decode_output_size` can be used.
 *
 ...
 *
 * The string of length `plain_len` has to be given in `plain_input`. The output will be placed in
 * `output`, and the length of the encoded output will be written to `output_len`. A trailing 0 byte
 * is NOT written. The user must ensure that `output` is big enough. For finding out how much space
 * is required for the output, the function `e_cobsr_encode_output_size` can be used.
 *
 * On success, E_COBSR_OK is returned. For invalid data or invalid parameters, an error is returned.
 */
E_Cobsr_Result
